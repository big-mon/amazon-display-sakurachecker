...
async function checkSakuraScore(productURL, asin) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            const scoreRating = window.ScoreParser ? window.ScoreParser.parseScoreRating(html) : null;
            const sakuraPercentage = window.ScoreParser ? window.ScoreParser.parseSakuraPercentage(html) : null;
